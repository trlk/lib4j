...
public class XDocument extends XNode {
	...
	public XDataNode eval(String expr) {
		...
		if (expr.charAt(0) != '/') {
			throw new RuntimeError("Evaluation error: %s", expr);
		} else {
		...
		if (!getRootName().equals(parts[0])) {
			throw new RuntimeError("Root node evaluation error: %d:%s", 0, parts[0]);
	...
	public void merge(XDocument doc) {

		getRoot().merge(doc.getRoot());
	}
	...
	public String toString() {
		...
		return Text.sprintf("%s=%s", root.getName(), root.toString());
	}
